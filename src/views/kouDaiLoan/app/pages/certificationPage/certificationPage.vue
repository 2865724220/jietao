<template>
    <div class="certif">
        ...
        <ul class="forms">
            <li>
                <label>持卡人</label>
                <input type="text" maxlength="10" placeholder="请输入持卡人姓名" v-model="postData.name" required/>
            </li>
            <li>
                <label>身份证号</label>
                <input type="text" placeholder="请输入身份证号" v-on:input="intelIdCard" maxlength="18" v-model="postData.id_card" required/>
                <img @click="postData.id_card=''" class="delinput" src="~assets/kouDaiLoan/app/images/closed.png" v-show="postData.id_card" />
            </li>
            ...
            <li>
                <label>银行卡号</label>
                <input type="number" v-on:input="intelBankCard" placeholder="请输入银行卡号" v-model="postData.card" required/>
        ...
        <div class="btnOut">
            <van-button type="danger" size="large" class="btn"  @click="bindCard" >确认绑卡</van-button>
        </div>
    ...
    export default {
               ...
               /* let sStr= window.location.search;
                ...
                myFetch(this.postData,'api/web/user-card/card-save',(res)=>{
                    if(res.code=='0'){
                        Toast('绑卡成功');
                        if(this.isPsw=='2'){
                            this.$router.replace('/trader-psw');
                        }else{
                            this.$router.go(-1);
                        }
        ...
        mounted () {
            this.$nextTick(function () {
                wait = 60;
                this.isPsw = this.$route.query.isPsw;
                let _this = this;
                myFetch({},'api/web/user/user-info',res=>{
                    _this.isPsw = res.data.set_pay_pwd;
                    if(res.data.id_card){
                       //_this.$router.go(-1)
                    }
                })
